...
module.exports = async (req, res) => {
  ...
  for (let i = 0; i < filteredUrls.length; i++) {
    ...
    const date = await page.evaluate(() => {
      return Array.from(
        document.getElementsByClassName("job-items"),
        (ul) => ul.children[0].innerText
      );
    });
        ...
        for (let j = 0; j < records.length; j++) {
          if (urls.indexOf(records[j].url) === -1) {
            db.Petrochemicals.updateOne(
              { url: records[j].url },
              { deleted: true }
            ).catch((err) => console.log("ERROR DELETING URL LINE 97", err));
          }
        }
      })
      .catch((err) =>
        console.log("ERROR FINDING PETROCHEMICAL RECORDS LINE 100", err)
      );
  }
};
